...
4. **Polish the AI Suggest feature.** The Create Task form already includes a "✨ Suggest with AI" button that asks the local stub provider in `lib/ai.ts` to propose a title, priority, and assignee from whatever the user has typed so far (the stub returns structured JSON streamed token-by-token, with simulated latency and a small failure rate). Treat the current implementation as MVP scope and make it production-ready: surface the streaming response as it arrives, handle errors with a retry affordance, cancel any in-flight request when the user retries, and decide what to do when the user has typed nothing. You may keep the stub provider as-is — the goal is the integration quality, not the model. Cover the new behavior with tests.
